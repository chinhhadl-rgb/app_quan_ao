...
public class AdminController {
    ...
    public String dashboard(HttpSession session, Model model) {
        ...
        Map<String, Object> totals = jdbcTemplate.queryForMap("""
            select 
                count(distinct case when order_status = 'Delivered' then id end) as total_orders,
                coalesce(sum(case when order_status = 'Delivered' then total_final end), 0) as total_revenue,
        ...
        model.addAttribute("topProducts", jdbcTemplate.queryForList("""
            select p.name, sum(od.quantity) as sold_count, sum(od.quantity * od.price) as total_earned
        ...
        // 4. Đơn hàng mới chờ duyệt
        model.addAttribute("pendingOrders", jdbcTemplate.queryForList("""
            select o.*, u.username
            from orders o
    ...
    // --- Quản lý Yêu cầu Hoàn trả (Returns) ---
    @GetMapping("/admin/returns")
    public String listReturns(HttpSession session, Model model) {
        if (!isAdmin(session)) return "redirect:/login";
        
        model.addAttribute("returns", jdbcTemplate.queryForList("""
            select r.*, u.username, o.total_final
            from returns r
    ...
    @PostMapping("/admin/returns/update")
    public String updateReturnStatus(
            @RequestParam int returnId, 
        ...
        jdbcTemplate.update("update returns set status = ?, admin_note = ? where id = ?", 
